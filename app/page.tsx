...
import { 
  ...
  Droplets, ArrowRight, Copy, Send, Loader2, Plus, Activity, Share2, LogOut, Download
} from 'lucide-react';
import { analyzeDocument, chatWithDocument } from '../lib/api';
...
export default function App() {
  ...
  // Convert jargonMap object to array for rendering
  const getJargonArray = (jargonMap: any) => {
    if (!jargonMap) return [];
    if (Array.isArray(jargonMap)) return jargonMap;
    return Object.entries(jargonMap).map(([term, explanation]) => ({ term, explanation }));
  };

  // Normalize followUp to always be a flat array
  const getFollowUpArray = (followUp: any) => {
    if (!followUp) return [];
    if (Array.isArray(followUp)) return followUp;
    // If object with tests/diet/activity
    const items: string[] = [];
    if (followUp.tests) items.push(...followUp.tests);
    if (followUp.diet) items.push(...followUp.diet);
    if (followUp.activity) items.push(...followUp.activity);
    return items;
  };
  ...
  const handleAnalyze = async (selectedFile: File | null, text: string = '') => {
    ...
    try {
      ...
      console.error(error);
      setErrorMsg(error?.message || 'Failed to analyze document. Please try again.');
      setFile(null);
    } finally {
      setLoading(false);
      setLoadingStep('');
  ...
  const handleSendChat = async () => {
    ...
    try {
      const groqHistory = newHistory.map(m => ({
  ...
  const handleCopyFamilySummary = async () => {
    if (!result?.familySummary) {
      showToast('No family summary available yet.', 'info');
      return;
    }

    try {
      await navigator.clipboard.writeText(result.familySummary);
      showToast('Family summary copied!');
    } catch (error) {
      showToast('Unable to copy summary.', 'info');
    }
  };

  const handleDownloadReport = () => {
    if (!result) {
      showToast('No report available to download.', 'info');
      return;
    }

    const followUpItems = getFollowUpArray(result.followUp);
    const jargonItems = getJargonArray(result.jargonMap);
    ...
    const warnings = Array.isArray(result.doctorCallWarnings) ? result.doctorCallWarnings : [];

    const reportText = [
      'MEDYRAX CLINICAL REPORT',
      `Generated: ${new Date().toLocaleString()}`,
      `Document Type: ${result.documentType || 'Unknown'}`,
      '',
      'Diagnosis',
      `${result.diagnosis || 'Not available'}`,
      '',
      'Family Summary',
      `${result.familySummary || 'Not available'}`,
      '',
      'Medications',
      ...(medications.length > 0
        ? medications.map((med: any, idx: number) => `${idx + 1}. ${med.name || 'Unknown'} | Dose: ${med.dose || 'N/A'} | Timing: ${med.timing || 'N/A'} | Days: ${med.days || 'N/A'}`)
        : ['No medications found.']),
      '',
      'Side Effects',
      ...(sideEffects.length > 0 ? sideEffects.map((effect: string, idx: number) => `${idx + 1}. ${effect}`) : ['No side effects listed.']),
      '',
      'Doctor Call Warnings',
      ...(warnings.length > 0 ? warnings.map((warning: string, idx: number) => `${idx + 1}. ${warning}`) : ['No urgent warnings listed.']),
      '',
      'Follow Up',
      ...(followUpItems.length > 0 ? followUpItems.map((item: string, idx: number) => `${idx + 1}. ${item}`) : ['No follow-up instructions listed.']),
      '',
      'Medical Terms Decoded',
      ...(jargonItems.length > 0
        ? jargonItems.map((item: any, idx: number) => `${idx + 1}. ${item.term}: ${item.explanation}`)
        : ['No medical terms decoded.'])
    ].join('\n');

    const docTypeSlug = String(result.documentType || 'report').replace(/\s+/g, '-').toLowerCase();
    const blob = new Blob([reportText], { type: 'text/plain;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `medyrax-${docTypeSlug}-${Date.now()}.txt`;
    ...
    <div className="flex h-screen bg-[#F5F7FA] font-sans text-slate-800 overflow-hidden">
                      ...
                      {showTextInput && (
                        <div className="mb-4">
                          <textarea 
                            ...
                            placeholder="Type or paste prescription / report details here..."
                      ...
                      >
                        ...
                        {loading ? (
                          <div className="flex flex-col items-center">
                            ...
                            <p className="text-[#1A36A8] font-medium">{loadingStep || 'Analyzing document with Medyrax AI...'}</p>
                          </div>
                        ) : (
                          <>
                            ...
                            <p className="text-slate-500 text-sm max-w-sm mb-8">
                              Supported: PDF, JPEG, PNG — Prescription, Lab Report, MRI, ECG, X-Ray and more.<br/>
                            </p>
                          ...
                          <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-slate-500">Uploaded just now • {(file.size / 1024 / 1024).toFixed(2)} MB • {result.documentType}</p>
                              </div>
                            </div>
                      ...
                      <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="text-sm font-bold text-slate-800 capitalize">{result.documentType}</span>
                          </div>

                          {/* Document Quality */}
                          <div className="flex justify-between items-center">
                            <span className="text-sm text-slate-500">Document Quality</span>
                            <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                              result.documentQuality === 'good' ? 'bg-green-100 text-green-700' :
                              result.documentQuality === 'low_quality' ? 'bg-orange-100 text-orange-700' :
                              'bg-red-100 text-red-700'
                            }`}>
                              {result.documentQuality === 'good' ? 'Good' : result.documentQuality === 'low_quality' ? 'Low Quality' : 'Too Short'}
                            </span>
                          </div>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className={`text-xs font-bold px-2 py-1 rounded-full ${
                              ...
                              result.confidence === 'medium' ? 'bg-orange-100 text-orange-700' :
                              'bg-red-100 text-red-700'
                            }`}>
                          ...
                          {result.coverage && (
                            <div>
                              <div className="flex justify-between text-sm mb-1">
                                ...
                                <span className="font-bold text-slate-800">{result.coverage.found}/{result.coverage.total} ({result.coverage.percentage}%)</span>
                              </div>
                              <div className="w-full bg-slate-100 rounded-full h-2">
                                <div 
                                  className={`h-2 rounded-full ${result.coverage.percentage >= 70 ? 'bg-green-500' : result.coverage.percentage >= 40 ? 'bg-orange-400' : 'bg-red-400'}`}
                                  style={{ width: `${result.coverage.percentage}%` }}
                                ></div>
                              </div>
                            </div>
                          )}
                          ...
                          {/* Medication Safety */}
                          {result.medicationWarnings && result.medicationWarnings.length > 0 && (
                            <div>
                              <p className="text-sm text-slate-500 mb-2">Medication Fields</p>
                              {result.medicationWarnings.map((med: any, idx: number) => (
                                <div key={idx} className="flex items-center gap-2 text-xs mb-1">
                                  <span className="font-bold text-slate-700">Med {med.index}:</span>
                      ...
                      <div className="space-y-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-slate-500">Groq llama-3.3-70b ready.</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <div className="w-2 h-2 rounded-full bg-green-500 mt-1.5"></div>
                          <div>
                            <h4 className="text-sm font-bold text-slate-800">Document Grounded</h4>
                            <p className="text-xs text-slate-500">AI answers only from your document.</p>
                          </div>
                        </div>
                        <div className="flex items-start gap-3">
                          <div className="w-2 h-2 rounded-full bg-green-500 mt-1.5"></div>
                          <div>
                            <h4 className="text-sm font-bold text-slate-800">Temperature 0.1</h4>
                            <p className="text-xs text-slate-500">Maximum faithfulness, no hallucination.</p>
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="lg:col-span-2 space-y-6">
                          ...
                          <table className="w-full text-left">
                            <thead>
                              <tr className="border-b border-slate-100">
                                ...
                                <th className="px-6 py-4 text-xs font-bold text-slate-400 uppercase tracking-wider">Dose</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                              {result.medications?.map((med: any, idx: number) => {
                                ...
                                const hasM = t.includes('morning') || t.includes('breakfast') || t.includes('once daily');
                                const hasA = t.includes('afternoon') || t.includes('lunch') || t.includes('twice') ;
                                const hasE = t.includes('evening');
                                const hasN = t.includes('night') || t.includes('bed') || t.includes('twice');
                                ...
                                return (
                                  <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-6 py-5 font-medium text-slate-700">{med.dose}</td>
                    ...
                    {result.sideEffects && result.sideEffects.length > 0 && (
                      <div className="bg-orange-50 rounded-2xl p-6 border border-orange-100 flex gap-4">
                        ...
                        <div>
                          <h4 className="font-bold text-orange-800 mb-1">Side Effect Warning</h4>
                    ...
                    <div className="bg-[#0B173E] rounded-3xl p-8 text-white shadow-lg">
                      ...
                      <div className="bg-white/10 rounded-2xl p-4 mb-6 flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-blue-200">{result.medications?.[0]?.dose || ''} • Upcoming</p>
                        </div>
                      </div>
                      <button 
              ...
              <div className="animate-in fade-in duration-300">
                <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex items-center gap-4 flex-wrap">
                    ...
                    <span className="text-sm text-slate-500 font-medium capitalize">{result.documentType} • {file?.name || 'Document'}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <button
                      onClick={handleDownloadReport}
                      className="bg-[#0B173E] text-white px-4 py-2.5 rounded-xl font-bold text-sm hover:bg-[#1A36A8] transition-colors flex items-center gap-2"
                    >
                      <Download className="w-4 h-4" /> Download Report
                    </button>
                    <button
                      onClick={handleCopyFamilySummary}
                      className="bg-slate-100 text-slate-800 px-4 py-2.5 rounded-xl font-bold text-sm hover:bg-slate-200 transition-colors flex items-center gap-2"
                    >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                      ...
                      <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100 flex flex-col">
                        ...
                        <div className="space-y-2 flex-1">
                          {getFollowUpArray(result.followUp).length > 0 ? (
                            getFollowUpArray(result.followUp).map((item: string, i: number) => (
                            ...
                            <p className="text-sm text-slate-400">Not mentioned in your document.</p>
                          )}
                        </div>
                        <button 
                          onClick={() => setActiveTab('chatbot')}
                          className="bg-[#1A36A8] text-white py-3 rounded-xl font-bold hover:bg-[#12267a] transition-colors w-full shadow-sm mt-6"
                        >
                          Ask AI About This
                        </button>
                      </div>
                  ...
                  <div className="space-y-6 flex flex-col h-full">
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100 flex-1">
                      ...
                      <div className="space-y-6">
                        {getJargonArray(result.jargonMap).slice(0, 4).map((item: any, idx: number) => (
                          <div key={idx}>
                            <h4 className="font-bold text-slate-800 mb-1">{item.term}</h4>
                            <p className="text-sm text-slate-500 leading-relaxed">{item.explanation as string}</p>
                          </div>
                        ))}
                        {getJargonArray(result.jargonMap).length === 0 && (
                          <p className="text-sm text-slate-400">No medical terms found.</p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-3xl p-6 shadow-sm border border-slate-100">
                      <div className="flex items-start gap-3 mb-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500 mt-1">Ask AI anything about this document.</p>
                        </div>
                      </div>
                      <button 
                        onClick={() => setActiveTab('chatbot')}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  <div className="lg:col-span-2 space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {result.sideEffects.map((effect: string, idx: number) => (
                            <div key={idx} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 border-l-4 border-l-orange-400">
                              <p className="text-sm text-slate-600 mb-4">{effect}</p>
                              <span className="bg-orange-50 text-orange-700 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider">Watch For This</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-slate-100">
                      ...
                      <div className="space-y-6">
                        {result.doctorCallWarnings && result.doctorCallWarnings.length > 0 ? (
                          ...
                          <p className="text-sm text-slate-400">No urgent warnings mentioned in your document.</p>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    <div className="bg-[#0B173E] rounded-3xl p-6 text-white shadow-lg">
                      ...
                      <div className="mb-6">
                        <span className="text-[10px] font-bold text-blue-300 uppercase tracking-widest block mb-1">Doctor WhatsApp</span>
                        ...
                        >
                          <MessageSquare className="w-4 h-4" /> WhatsApp Doctor
                        </button>
                      </div>
                    ...
                    {chatHistory.length === 0 ? (
                      <div className="h-full flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-slate-500 max-w-md">
                          {result ? 'Ask me anything about your uploaded document.' : 'Upload a document first, then ask questions here.'}
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-6">
                  ...
                  <div className="p-4 border-t border-slate-100 bg-slate-50">
                    <div className="relative max-w-4xl mx-auto">
                      <input 
                        ...
                        placeholder={result ? "Ask a question about your document..." : "Upload a document first..."}
                        disabled={!result}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <input 
                type="text" 
                placeholder="Doctor's WhatsApp Number" 
                value={doctorNumber}
                onChange={(e) => setDoctorNumber(e.target.value)}
                className="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-red-400"
              />
              <button 
                onClick={() => doctorNumber ? window.open(`https://wa.me/${doctorNumber}`, '_blank') : alert('Enter a number first.')}
                className="w-full bg-green-500 text-white py-2 rounded-lg font-bold hover:bg-green-600 transition-colors text-sm"
              >
                Message on WhatsApp
              </button>
              <button 
                onClick={() => window.open('tel:108')}
                className="w-full bg-red-500 text-white py-2 rounded-lg font-bold hover:bg-red-600 transition-colors text-sm"
              >
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="font-bold text-slate-800">Language</h4>
                  <p className="text-sm text-slate-500">AI response language</p>
                </div>
                <button
                  onClick={() => setLanguage(language === 'english' ? 'hindi' : 'english')}
                  className="px-4 py-2 bg-[#1A36A8] text-white rounded-lg text-sm font-bold"
                >
      ...
      {showHelp && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center p-6 border-b border-slate-100">
              <h3 className="text-xl font-bold text-[#0B173E] flex items-center gap-2 font-headline"><HelpCircle className="w-5 h-5"/> Help</h3>
              <button onClick={() => setShowHelp(false)} className="text-slate-400 hover:text-slate-600">
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
              </button>
            </div>
            <div className="p-6 space-y-4 text-sm text-slate-600">
              <p>1. Upload a prescription, lab report, MRI, ECG, or any medical document.</p>
              <p>2. AI extracts key information in plain language.</p>
              <p>3. Use the chatbot to ask questions about your document.</p>
              <p>4. Everything is grounded in your document — AI never adds outside information.</p>
            </div>
          </div>
        </div>
      )}
              ...
              {result ? (
                <div className="flex gap-4 items-start p-3 bg-blue-50 rounded-xl">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-600 capitalize">{result.documentType} analyzed successfully.</p>
